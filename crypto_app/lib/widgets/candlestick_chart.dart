...
// Same hand-rolled candlestick painter as the main app
// (mobile_app/lib/widgets/candlestick_chart.dart) - copied, with
// formatRupees() swapped for this app's own formatUsd() (crypto
// premiums are USD, not Rupees). See that file's own comments for the
// reasoning behind each fix baked into this painter (flat-candle
// floor, volume strip, etc.) - kept verbatim since they're real
// bugs-found-live, not stylistic choices.
        ...
        children: [
          for (var i = 0; i <= _levels; i++)
            Positioned(
              ...
              left: 6,
              child: Text(
                formatUsd(maxPrice - (i / _levels) * (maxPrice - minPrice)),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final line in referenceLines) {
      ...
      final textPainter = TextPainter(
        text: TextSpan(
          text: ' ${line.label} ${formatUsd(line.price)}',
